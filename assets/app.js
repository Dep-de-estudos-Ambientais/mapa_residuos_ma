...
const FORM_API_URL = String(window.APP_CONFIG?.GOOGLE_APPS_SCRIPT_URL || "").trim();
...
const state = {
  ...
  decorators: [],
  remoteIrregularLastUpdate: null
};
...
function irregularPopup(p){
  const isForm = p.origem === "questionario";
  const rows = isForm ? [
    ["Código", p.codigo_ponto],
    ["Identificação", p.identificacao_ponto],
    ["Município", p.municipio],
    ["Data da vistoria", p.data_vistoria],
    ["Próximo de", p.proximo_de],
    ["Distância aproximada", p.distancia_km ? `${p.distancia_km} km` : ""],
    ["Há pessoas na área?", p.pessoas_na_area],
    ["Identificação de pessoas", p.identificacao_pessoas],
    ["Recursos naturais próximos", p.recursos_naturais],
    ["Plantios ou hortas próximos", p.plantios_hortas],
    ["Empreendimentos próximos", p.empreendimentos_proximos],
    ["Situação do ponto", p.situacao_ponto],
    ["Condição de acesso", p.condicao_acesso],
    ["Tipos de resíduos", p.tipos_residuos],
  ...
  return `<div class="popup-title popup-warning">Área de descarte irregular</div>
    <div class="popup-grid">${visibleRows.map(([k,v])=>`<b>${escapeHtml(k)}</b><span>${escapeHtml(v)}</span>`).join("")}</div>
    <span class="group-badge">${escapeHtml(displayGroup(p.agrupamento))}</span>`;
}

function loadJSONP(url, timeoutMs=15000){
  return new Promise((resolve,reject)=>{
    if(!url) return reject(new Error("URL do Google Apps Script não configurada."));
    const callback = `__descarteGeoJSON_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    const script = document.createElement("script");
    const timer = setTimeout(()=>cleanup(new Error("Tempo esgotado ao consultar os cadastros.")), timeoutMs);
    const cleanup = err => {
      clearTimeout(timer);
    ...
    script.src = `${url}${sep}action=geojson&callback=${encodeURIComponent(callback)}&_=${Date.now()}`;
    script.async = true;
    script.onerror = () => cleanup(new Error("Falha ao consultar o Google Apps Script."));
    document.head.appendChild(script);
  });
}

function clearRemoteIrregular(){
  if(!state.layers.irregular) return;
  const remove = [];
  state.layers.irregular.eachLayer(layer=>{
    if(layer.__source === "questionario") remove.push(layer);
  });
  remove.forEach(layer=>state.layers.irregular.removeLayer(layer));
}

function addRemoteIrregularFeatures(geojson){
  if(!geojson || geojson.type !== "FeatureCollection" || !Array.isArray(geojson.features)) return 0;
  clearRemoteIrregular();
  let count = 0;

  geojson.features.forEach(feature=>{
    const geom = feature?.geometry;
    const p = feature?.properties || {};
    if(!geom || !["Point","Polygon"].includes(geom.type)) return;
    const group = p.agrupamento || "";

    if(geom.type === "Point"){
      const [lon,lat] = geom.coordinates || [];
      if(!Number.isFinite(Number(lon)) || !Number.isFinite(Number(lat))) return;
      const marker = L.marker([lat,lon], {icon:svgWarningIcon(), zIndexOffset:820})
        .bindPopup(irregularPopup(p), {maxWidth:380});
      marker.__group = group;
      marker.__source = "questionario";
    ...
    if(geom.type === "Polygon"){
      const rings = (geom.coordinates || []).map(ring =>
        (ring || []).map(pair => [Number(pair[1]), Number(pair[0])])
      ).filter(ring => ring.length >= 4);
      if(!rings.length) return;

      const polygon = L.polygon(rings, {
        color:"#b91c1c", weight:2.2, opacity:.95,
        fillColor:"#dc2626", fillOpacity:.22
      }).bindPopup(irregularPopup(p), {maxWidth:380});
      polygon.__group = group;
      polygon.__source = "questionario";
      polygon.addTo(state.layers.irregular);

      const center = polygon.getBounds().getCenter();
      const marker = L.marker(center, {icon:svgWarningIcon(), zIndexOffset:830})
        .bindPopup(irregularPopup(p), {maxWidth:380});
      marker.__group = group;
      marker.__source = "questionario";
      marker.addTo(state.layers.irregular);
  ...
  state.remoteIrregularLastUpdate = new Date();
  updateVisibility();
  return count;
}

async function refreshRemoteIrregular({silent=false}={}){
  if(!FORM_API_URL || !state.layers.irregular) return 0;
  try{
    const geojson = await loadJSONP(FORM_API_URL);
    const count = addRemoteIrregularFeatures(geojson);
    if(!silent) showToast(`${count} cadastro(s) do questionário carregado(s).`);
    return count;
  }catch(err){
    console.warn("Cadastros do questionário não carregados:", err);
    if(!silent) showToast("Não foi possível atualizar os cadastros do questionário.");
    return 0;
  }
}
...
function bindUI(){
  ...
  // Os registros do formulário são consultados somente no carregamento da página.
  // Para visualizar novos cadastros, basta atualizar/recarregar o WebGIS.
}
...
function syncResponsiveLayout(){
  ...
  if(mobile){
    ...
    if(header) root.style.setProperty("--mobile-header-height", `${Math.ceil(header.getBoundingClientRect().height)}px`);
    if(footer) root.style.setProperty("--mobile-footer-height", `${Math.ceil(footer.getBoundingClientRect().height)}px`);
  } else {
...
function installResponsiveMapFix(){
  let timer;
  const schedule = (delay=0)=>{
    clearTimeout(timer);
    timer = setTimeout(syncResponsiveLayout, delay);
  };
  window.addEventListener("resize", ()=>schedule(40), {passive:true});
  window.addEventListener("orientationchange", ()=>schedule(260), {passive:true});
  if(window.visualViewport){
    window.visualViewport.addEventListener("resize", ()=>schedule(60), {passive:true});
  }
  if(window.ResizeObserver){
    const observer = new ResizeObserver(()=>schedule(20));
    const mapEl = document.getElementById("map");
    const header = document.querySelector(".topbar");
    const footer = document.querySelector(".institutional-footer");
    [mapEl, header, footer].filter(Boolean).forEach(el=>observer.observe(el));
  }
  schedule(0);
  schedule(300);
}
...
async function init(){
  try{
    await loadData();
    buildLayers();
    refreshRemoteIrregular({silent:true});
    populateFilter();
    renderLegend();
    bindUI();
    installResponsiveMapFix();
    fitActiveGroup();
