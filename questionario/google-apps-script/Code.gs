...
function setupProject() {
  ...
  const wktBackfilled = backfillWkt_(sheet);
...
function doPost(e) {
  ...
  try {
    ...
    const row = [
      ...
      geometryToWkt_(payload.geometryGeoJSON)
    ];
...
function doGet(e) {
  ...
  // Endpoint geográfico público para o WebGIS e o QGIS.
  // Exemplo: /exec?action=geojson
  if (action === "geojson") {
    const geojson = buildGeoJSON_();
    if (callback) {
      return ContentService
        .createTextOutput(callback + "(" + JSON.stringify(geojson) + ");")
...
function buildGeoJSON_() {
  const sheet = getSheet_();
  ensureHeaders_(sheet);
  const lastRow = sheet.getLastRow();
  if (lastRow < 2) return { type: "FeatureCollection", features: [] };
  ...
  values.forEach(function(row) {
    const rawGeometry = row[index["Geometria (GeoJSON)"]];
    if (!rawGeometry) return;

    let geometry;
    try { geometry = JSON.parse(rawGeometry); }
    catch (_) { return; }
    if (!geometry || ["Point", "Polygon"].indexOf(geometry.type) < 0) return;

    const props = {
      origem: "questionario",
      id_registro: row[index["ID do registro"]],
      codigo_ponto: row[index["Código do ponto"]],
      identificacao_ponto: row[index["Identificação do ponto"]],
      data_vistoria: row[index["Data da vistoria"]],
      municipio: row[index["Município"]],
      agrupamento: row[index["Agrupamento"]],
      proximo_de: row[index["Próximo de"]],
      distancia_km: row[index["Distância aproximada (km)"]],
      pessoas_na_area: row[index["Há pessoas na área?"]],
      identificacao_pessoas: row[index["Como foi identificada a existência de pessoas?"]],
      recursos_naturais: row[index["Recursos naturais próximos"]],
      plantios_hortas: row[index["Áreas de plantio ou hortas próximas?"]],
      empreendimentos_proximos: row[index["Empreendimentos próximos?"]],
      situacao_ponto: row[index["Situação aparente do ponto"]],
      condicao_acesso: row[index["Condição de acesso"]],
      tipos_residuos: row[index["Tipos de resíduos observados"]],
      observacoes: row[index["Observações"]],
      tipo_geometria: row[index["Tipo de geometria"]],
      latitude_referencia: row[index["Latitude de referência"]],
      longitude_referencia: row[index["Longitude de referência"]],
      precisao_gps_m: row[index["Precisão do GPS (m)"]],
      fotografias: row[index["Fotografias"]],
      geometria_wkt: row[index["Geometria (WKT)"]] || geometryToWkt_(rawGeometry)
    };

    features.push({
      type: "Feature",
      id: props.id_registro || undefined,
      geometry: geometry,
      properties: props
    });
  });
...
function geometryToWkt_(geometryGeoJSON) {
  let geometry = geometryGeoJSON;
  if (typeof geometryGeoJSON === "string") {
    try { geometry = JSON.parse(geometryGeoJSON); }
    catch (_) { return ""; }
  }
  if (!geometry || !geometry.type) return "";

  if (geometry.type === "Point" && Array.isArray(geometry.coordinates)) {
    return "POINT (" + formatCoordinatePair_(geometry.coordinates) + ")";
  }

  if (geometry.type === "Polygon" && Array.isArray(geometry.coordinates)) {
    const rings = geometry.coordinates.map(function(ring) {
      return "(" + ring.map(formatCoordinatePair_).join(", ") + ")";
    });
...
function formatCoordinatePair_(pair) {
  if (!Array.isArray(pair) || pair.length < 2) return "";
  return formatNumber_(pair[0]) + " " + formatNumber_(pair[1]);
}
...
function backfillWkt_(sheet) {
  if (sheet.getLastRow() < 2) return 0;
  const geoCol = HEADERS.indexOf("Geometria (GeoJSON)") + 1;
  const wktCol = HEADERS.indexOf("Geometria (WKT)") + 1;
  if (!geoCol || !wktCol) return 0;

  const count = sheet.getLastRow() - 1;
  const geoValues = sheet.getRange(2, geoCol, count, 1).getDisplayValues();
  const currentWkt = sheet.getRange(2, wktCol, count, 1).getDisplayValues();
  let changed = 0;
  const output = geoValues.map(function(row, i) {
    const existing = String(currentWkt[i][0] || "").trim();
    if (existing) return [existing];
    const wkt = geometryToWkt_(row[0]);
    if (wkt) changed++;
    return [wkt];
  });
  sheet.getRange(2, wktCol, count, 1).setValues(output);
  return changed;
}
